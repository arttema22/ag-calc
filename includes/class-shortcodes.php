...
class AG_Shortcodes
{
    ...
    public function enqueue_frontend_assets()
    {
        ...
        // Скрипт калькулятора
        wp_enqueue_script('ag-calc-js', AG_CALC_PLUGIN_URL . 'assets/js/frontend-calculator.js', ['jquery'], AG_CALC_VERSION, true);
        ...
        switch ($type) {
            ...
            case 'radio':
                ...
                if (!empty($config['options'])) {
                    foreach ($config['options'] as $i => $option) {
                        $price = floatval($option['price'] ?? 0);
                        $html .= '<div class="ag-radio-item">';
                        $html .= '<input type="radio"
                                    name="' . esc_attr($key) . '"
                                    class="ag-field-radio-input"
                                    data-field-key="' . esc_attr($key) . '"
                                    data-price="' . esc_attr($price) . '"
                                    value="' . esc_attr($option['value']) . '"
                                    id="' . esc_attr($key . '_' . $i) . '"
                                    ' . checked($i, 0, false) . '>';
                        $html .= '<label class="ag-radio-label" for="' . esc_attr($key . '_' . $i) . '">' .
            ...
            case 'checkbox':
                ...
                $html .= '<input type="checkbox"
                            name="' . esc_attr($key) . '"
                            class="ag-field-checkbox"
                            data-field-key="' . esc_attr($key) . '"
                            value="on" id="' . esc_attr($key) . '">';
                $html .= '<label class="ag-checkbox-label" for="' . esc_attr($key) . '">' .
